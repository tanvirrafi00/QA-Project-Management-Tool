...
/**
 * PasteBugsDialog — Enhanced modal dialog for bug import with comprehensive user guidance.
...
import { parsePaste, savePaste, type BugPasteRow } from '../services/bug-paste.service';
...
export function PasteBugsDialog({ projectName, module = '', open, onClose, onSaved }: Props) {
    ...
    // Required columns for bug import
    ...
    const handlePaste = async () => {
        ...
        try {
            ...
            toast.success(`Parsed ${result.data.totalBugs} bugs successfully`);
        } catch (err: any) {
    ...
    const handleSave = async () => {
        if (!preview || !preview.bugs || preview.bugs.length === 0) {
            setError('No bugs to save.');
            return;
        }
        ...
        try {
            ...
            if (!result.success) {
                setError(result.error || 'Failed to save bugs');
                return;
            }

            if (result.data) {
                toast.success(`Successfully imported ${result.data.total} bugs`);
            }
            onSaved();
            onClose();
        } catch (err: any) {
            setError(err.message || 'Failed to save bugs');
            toast.error('Failed to save bugs');
        } finally {
            setSaving(false);
        }
    };
    ...
    return (
        <Modal open={open} onClose={() => !saving && onClose()} size="lg" title="Paste Bugs">
            {!preview ? (
                <div className="space-y-6">
                    {/* Main Instructions */}
                    <div className="bg-[#F8FAFC] rounded-lg p-4 border border-[#E2E8F0]">
                        <p className="text-sm text-[#1E293B] mb-4">
                            Import bugs by pasting a table copied from AI, Excel or Google Sheets.
                        </p>
                        ...
                        {showTemplate && (
                            <div className="bg-white rounded border border-[#E2E8F0] p-3">
                                <div className="text-xs font-mono text-[#1E293B] mb-2 overflow-x-auto">
                                    <pre>| Bug ID | Module | Title / Summary | Severity | Priority | Description | Steps to Reproduce | Expected Result | Actual Result | Bug Impact Area | Status | Assigned To |
                                        |--------|---------|-----------------|----------|----------|-------------|--------------------|-----------------|---------------|-----------------|--------|-------------|
                                        | BUG-001 | Login | Login button overlaps | High | P1 | Login button is misaligned | 1. Open login page&lt;br&gt;2. Observe button position | Button should be centered | Button appears shifted left | UI | Open | Unassigned |</pre>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                            ...
                            <div>
                                ...
                                {error.includes('Missing Required Column') && (
                                    <p className="text-xs text-red-600 mt-1">Please ensure all required columns are included in your header row.</p>
                                )}
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-purple-50 rounded-lg p-3 border border-purple-200">
                        <div className="flex items-start gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-purple-600">Generate bug or test case tables with AI, copy the generated table, then paste it here.</p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#F8FAFC] rounded-lg p-4 border border-[#E2E8F0]">
                        <p className="text-sm text-[#1E293B] mb-2">
                            <strong>{preview.totalBugs} Bugs</strong> parsed successfully
                        </p>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-[#E2E8F0]">
                                    <th className="text-left py-2 px-3 font-semibold text-[#1E293B]">Bug ID</th>
                                    <th className="text-left py-2 px-3 font-semibold text-[#1E293B]">Module</th>
                                    <th className="text-left py-2 px-3 font-semibold text-[#1E293B]">Title</th>
                                    <th className="text-left py-2 px-3 font-semibold text-[#1E293B]">Severity</th>
                            ...
                            <tbody>
                                {preview.bugs.map((bug: BugPasteRow, index: number) => (
                                    <tr key={index} className="border-b border-[#F1F5F9] hover:bg-[#F8FAFC]">
                                        <td className="py-2 px-3 text-[#64748B]">{bug.bugId || 'Auto-generated'}</td>
                                        <td className="py-2 px-3 text-[#1E293B]">{bug.module}</td>
                                        <td className="py-2 px-3 text-[#1E293B]">{bug.title}</td>
                                        <td className="py-2 px-3">
                                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${bug.severity === 'Critical' ? 'bg-red-100 text-red-700' :
                                        ...
                                        <td className="py-2 px-3 text-[#64748B]">{bug.priority}</td>
                                        <td className="py-2 px-3">
                                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${bug.status === 'Open' ? 'bg-blue-100 text-blue-700' :
                                                bug.status === 'Assigned' ? 'bg-purple-100 text-purple-700' :
                                                    bug.status === 'In Progress' ? 'bg-indigo-100 text-indigo-700' :
                                                        bug.status === 'Fixed' ? 'bg-green-100 text-green-700' :
                                                            bug.status === 'Verified' ? 'bg-teal-100 text-teal-700' :
